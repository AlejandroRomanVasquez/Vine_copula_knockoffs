...
# Vector with several M values
vec_M <- c(1, 5, 10, 20, 30, 40, 50)
...
for (M in vec_M) {
  for (i in 1:n_sim) {
    ...
    data <- data.frame(M = rep(M, n), Iteration = rep(i, n), y = y, X_mixed)
    data_betas <- data.frame(M = M, Iteration = i, matrix_beta)
...
df_column_names_dvine_results <- c("M", "Iteration", "Dvine_power", "Dvine_FDP")
...
for (idx in 1:n_simulations) {
  ...
  # D-vine fitting
  dvine_distributions <- X_Xk_dvine_distributions(X_cont = X_cont_dvine_order, vinecop_family, 
                                                  n_cores, psi0 = 0.95)
  ...
  time_ekn_dvines <- system.time(
    res_dvines <- ekn_dvines(X = X_dvine_order_matrix, X_cont = X_cont_dvine_order, 
                             ...
                             dvine_distributions, M = value, M_lasso = M_lasso,
  ...
  # Data frame with the information of this iteration
  data <- data.frame(M = value, Iteration = iteration, Dvine_power = dvine_Power, Dvine_FDP = dvine_FDP)
  
  # Merge the dataframes by stacking rows
  df_nondvine_results <- rbind(df_nondvine_results, data)
  
  cat(sprintf("The iteration is %d with M value of %.2f \n", iteration, value))
}
...
write.csv(df_nondvine_results, file = "df_nondvine_results_t_markov_M.csv", row.names = FALSE)

results_nondvine_t_markov <- df_nondvine_results %>% group_by(M) %>%
  summarise(meanPower = mean(Dvine_power),sdPower= sd(Dvine_power), meanFDP = mean(Dvine_FDP), sdFDP=sd(Dvine_FDP) )
View(results_nondvine_t_markov)
...
#Exporting the results to csv file
write.csv(df_dvine_results, file = "df_dvine_results_t_markov_M.csv", row.names = FALSE)

results_dvine_t_markov <- df_dvine_results %>% group_by(M) %>%
  summarise(meanPower = mean(Dvine_power),sdPower= sd(Dvine_power), meanFDP = mean(Dvine_FDP), sdFDP=sd(Dvine_FDP) )
View(results_dvine_t_markov)
...
df_column_names_cvine_results <- c("M", "Iteration", "Cvine_power", "Cvine_FDP")
...
for (idx in 1:n_simulations) {
  ...
  # C-vine fitting
  rvine_distributions <- X_Xk_rvine_distributions(X_cont = X_cont_cvine_order, vinecop_family, n_cores, psi0 = 0.95)
  ...
  time_ekn_cvines <- system.time(
    res_cvines <- ekn_cvines(X = X_cvine_order_matrix, X_cont = X_cont_cvine_order, 
                             column_type = column_type, y = y_matrix, 
                             rvine_distributions, M = value, M_lasso = M_lasso,
  ...
  # Data frame with the information of this iteration
  data <- data.frame(M = value, Iteration = iteration, Cvine_power = cvine_Power, 
                     Cvine_FDP = cvine_FDP)
...
#Exporting the results to csv file
write.csv(df_noncvine_results, file = "df_noncvine_results_t_markov_M.csv", row.names = FALSE)

results_noncvine_t_markov <- df_noncvine_results %>% group_by(M) %>%
  summarise(meanPower = mean(Cvine_power),sdPower=sd(Cvine_power), meanFDP = mean(Cvine_FDP), sdFDP=sd(Cvine_FDP) ) 
View(results_noncvine_t_markov) 
...
#Exporting the results to csv file
write.csv(df_cvine_results, file = "df_cvine_results_t_markov_M.csv", row.names = FALSE)

results_cvine_t_markov <- df_cvine_results %>% group_by(M) %>%
  summarise(meanPower = mean(Cvine_power),sdPower=sd(Cvine_power), meanFDP = mean(Cvine_FDP), sdFDP=sd(Cvine_FDP) ) 
View(results_cvine_t_markov) 
