...
#sample_ar1t()-->Samples t-distributed variables according to a Markov chain.
#The code to implement this function is adapted from:
...
# Vector with several psi0 values
vec_psi0 <- c(0.5, 0.6, 0.7, 0.8, 0.9)

# Proportion of ordinal variables
prop_ord <- 0.2
...
for (psi0 in vec_psi0) {
  for (i in 1:n_sim) {
    ...
    data <- data.frame(Psi0 = rep(psi0, n), Iteration = rep(i, n), y = y, X_mixed)
    data_betas <- data.frame(Psi0 = psi0, Iteration = i, matrix_beta)
...
vinecop_family <- 'nonparametric' # Class of bivariate copula 
M <- 50
M_lasso <- 10
...
df_column_names_dvine_results <- c("Psi0", "Iteration", "Dvine_power", "Dvine_FDP")
...
for (idx in 1:n_simulations) {
  ...
  # D-vine fitting
  dvine_distributions <- X_Xk_dvine_distributions(X_cont = X_cont_dvine_order, vinecop_family, n_cores, psi0=value)
  ...
  # Data frame with the information of this iteration
  data <- data.frame(Psi0 = value, Iteration = iteration, Dvine_power = dvine_Power, Dvine_FDP = dvine_FDP)
  
  # Merge the dataframes by stacking rows
  df_nondvine_results <- rbind(df_nondvine_results, data)
  
  cat(sprintf("The iteration is %d with psi0 value of %.2f \n", iteration, value))
}
...
#write.csv(df_nondvine_results, file = "df_nondvine_results_t_markov_psi0_n200.csv", row.names = FALSE)

results_nondvine_t_markov_n200 <- df_nondvine_results %>% group_by(Psi0) %>%
  summarise(meanPower = mean(Dvine_power),meanFDP = mean(Dvine_FDP) )
View(results_nondvine_t_markov_n200)
...
registerDoParallel(cl) # Registering a parallel backend for doParallel
alpha_dvine <- 0.2 # Target FDR
vinecop_family <- 'parametric' # Class of bivariate copula 
M <- 50
M_lasso <- 10
lasso_family <- 'gaussian'
n_folds <- 5 # Number of folds for Lasso CV
...
stopCluster(cl) #Stopping cluster
print("Time for running the parametric DTLDCKe procedure for the simulations:")
dvine_time <- end_time-start_time
print(dvine_time)

#Exporting the results to csv file
#write.csv(df_dvine_results, file = "df_dvine_results_t_markov_psi0_n200.csv", row.names = FALSE)

results_dvine_t_markov_n200<- df_dvine_results %>% group_by(Psi0) %>%
  summarise(meanPower = mean(Dvine_power),meanFDP = mean(Dvine_FDP) )
View(results_dvine_t_markov_n200)
...
df_column_names_cvine_results <- c("Psi0", "Iteration", "Cvine_power", "Cvine_FDP")
...
for (idx in 1:n_simulations) {
  ...
  # C-vine fitting
  rvine_distributions <- X_Xk_rvine_distributions(X_cont = X_cont_cvine_order, vinecop_family, n_cores, psi0=value)
  ...
  # Data frame with the information of this iteration
  data <- data.frame(Psi0 = value, Iteration = iteration, Cvine_power = cvine_Power, 
                     Cvine_FDP = cvine_FDP)
...
#Exporting the results to csv file
#write.csv(df_noncvine_results, file = "df_noncvine_results_t_markov_psi0_n200.csv", row.names = FALSE)
#df_noncvine_results_75_95_p100 <- df_noncvine_results

results_noncvine_t_markov_n200 <- df_noncvine_results %>% group_by(Psi0) %>%
  summarise(meanPower = mean(Cvine_power),meanFDP = mean(Cvine_FDP) )
View(results_noncvine_t_markov_n200 )
...
for (idx in 1:n_simulations) {
  ...
  cat(sprintf("The knockoff DTCLCke filter POWER %.2f%% with an FDP of %.2f%%\n", cvine_Power, cvine_FDP))
...
stopCluster(cl) #Stopping cluster
print("Time for running the DLCCKe procedure for the simulations:")
cvine_time <- end_time-start_time
print(cvine_time)

#Exporting the results to csv file
#write.csv(df_cvine_results, file = "df_cvine_results_t_markov_psi0_n200.csv", row.names = FALSE)

results_cvine_t_markov_n200 <- df_cvine_results %>% group_by(Psi0) %>%
  summarise(meanPower = mean(Cvine_power),meanFDP = mean(Cvine_FDP) )
View(results_cvine_t_markov_n200)
