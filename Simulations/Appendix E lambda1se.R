...
#lambda_type: "min" is for lambda.min, "1se" is for lambda.1se
#M_lasso: integer related to the number of runs for the stabilization against CV (Roberts and Nowak, 2014)
#n_folds: integer indicating the number of cross validations
#random_cv: logic value for a random assignation of the folds in the Cross-validation
#        TRUE is the default. FALSE is for reproducibility purposes. 
...
stable_lasso_glmnet <- function(X, y, lasso_family, lambda_type="min",
                                M_lasso = 10, n_folds = 5, 
                                random_cv=TRUE){
...
ekn_dvines <- function(X, X_cont, column_type, y, dvine_distributions, M=50, M_lasso=10, alpha=0.2, 
                       gamma=0.1, lasso_family, lambda_type="min",
                       n_cores=1, n_folds = 5, random_cv=TRUE, random_evalues=TRUE){
...
#### Non-parametric DTLDCKe with lambda 1se ####
...
for (idx in 1:n_simulations) {
  ...
  time_ekn_dvines <- system.time(
    res_dvines <- ekn_dvines(X = X_dvine_order_matrix, X_cont = X_cont_dvine_order, 
                             ...
                             lasso_family = lasso_family, lambda_type=lambda_type,
                             n_cores = n_cores, random_cv = FALSE, random_evalues = FALSE)
  )
...
write.csv(df_nondvine_results, file = "df_nondvine_results_Gaussian_X_lambda_1se.csv", row.names = FALSE)

results_nondvine_1se <- df_nondvine_results%>% group_by(Ordinals_Proportion) %>%
  summarise(meanPower = mean(Dvine_power),meanFDP = mean(Dvine_FDP) )
View(results_nondvine_1se)



#### Non-parametric DLCCKe with lambda 1se ####
...
#Exporting the results to csv file
write.csv(df_noncvine_results, file = "df_noncvine_results_Gaussian_X_lambda_1se.csv", row.names = FALSE)

results_noncvine_1se <- df_noncvine_results%>% group_by(Ordinals_Proportion) %>%
  summarise(meanPower = mean(Cvine_power),meanFDP = mean(Cvine_FDP) )
View(results_noncvine_1se)





#### Non-parametric DLDCKe with lambda min ####
...
#Exporting the results to csv file
write.csv(df_nondvine_results, file = "df_nondvine_results_Gaussian_X_lambda_min.csv", row.names = FALSE)
...
#### Non-parametric DLCCKe with lambda min ####
...
#Exporting the results to csv file
write.csv(df_noncvine_results, file = "df_noncvine_results_Gaussian_X_lambda_min.csv", row.names = FALSE)

results_noncvine <- df_noncvine_results%>% group_by(Ordinals_Proportion) %>%
  summarise(meanPower = mean(Cvine_power),meanFDP = mean(Cvine_FDP) )
