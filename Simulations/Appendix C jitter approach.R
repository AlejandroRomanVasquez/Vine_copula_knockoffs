...
#Ordinal to jitter transfomation
ordinal_to_continuous_jitter <- function(column) {
  
  if (is.null(column)) {
    stop("Argument column is missing")
  }
  
  # Treat variables with < 11 levels as ordinal
  if (length(unique(column)) < 11) {
  ...
  for (idx in 1:n_simulations) {
    ...
    time_ekn_dvines <- system.time(
      res_dvines <- ekn_dvines(X = X_dvine_order_matrix, X_cont = X_cont_dvine_order, 
                               ...
                               n_cores = n_cores, random_cv = TRUE, random_evalues = TRUE)
    )
...
nondvine_time_latent <- end_time-start_time
print(nondvine_time_latent)

#Exporting the results to csv file
write.csv(df_nondvine_results, file = "df_nondvine_results_Gaussian_X_latent.csv", row.names = FALSE)

results_nondvine_latent <- df_nondvine_results%>% group_by(Ordinals_Proportion) %>%
  summarise(meanPower = mean(Dvine_power),meanFDP = mean(Dvine_FDP) )
View(results_nondvine_latent)



#### Non-parametric DTDCKe jitter ####
...
for (idx in 1:n_simulations) {
  ...
  # Ordinal to continuous jitter  transformation
  X_cont_dvine_order <- apply(X = X_dvine_order, MARGIN = 2, FUN = ordinal_to_continuous_jitter)
...
stopCluster(cl) #Stopping cluster
print("Time for running the non-DTLDCKe procedure for the simulations:")
nondvine_time_jitter <- end_time-start_time
print(nondvine_time_jitter)

#Exporting the results to csv file
write.csv(df_nondvine_results, file = "df_nondvine_results_Gaussian_X_jitter.csv", row.names = FALSE)

results_nondvine_jitter <- df_nondvine_results%>% group_by(Ordinals_Proportion) %>%
  summarise(meanPower = mean(Dvine_power),meanFDP = mean(Dvine_FDP) )
View(results_nondvine_jitter)
...
dvine_time_latent <- end_time-start_time
print(dvine_time_latent)

#Exporting the results to csv file
write.csv(df_dvine_results, file = "df_dvine_results_Gaussian_X_latent.csv", row.names = FALSE)

dvine_results_latent <- df_dvine_results %>% group_by(Ordinals_Proportion) %>%
  summarise(meanPower = mean(Dvine_power),meanFDP = mean(Dvine_FDP) )
View(dvine_results_latent)

#### Parametric DTDCKe jitter ####
...
dvine_time_jitter <- end_time-start_time
print(dvine_time_jitter)

#Exporting the results to csv file
write.csv(df_dvine_results, file = "df_dvine_results_Gaussian_X_jitter.csv", row.names = FALSE)

dvine_results_jitter <- df_dvine_results %>% group_by(Ordinals_Proportion) %>%
  summarise(meanPower = mean(Dvine_power),meanFDP = mean(Dvine_FDP) )
View(dvine_results_jitter)
...
for (idx in 1:n_simulations) {
  ...
  # Data frame with the information of this iteration
  data <- data.frame(Ordinals_Proportion = value, Iteration = iteration, Cvine_power = cvine_Power, 
                     Cvine_FDP = cvine_FDP)
...
stopCluster(cl) #Stopping cluster
print("Time for running the non-DLCCKe procedure for the simulations:")
noncvine_time_latent <- end_time-start_time
print(noncvine_time_latent)

#Exporting the results to csv file
write.csv(df_noncvine_results, file = "df_noncvine_results_Gaussian_X_latent.csv", row.names = FALSE)

results_noncvine_latent <- df_noncvine_results%>% group_by(Ordinals_Proportion) %>%
  summarise(meanPower = mean(Cvine_power),meanFDP = mean(Cvine_FDP) )
View(results_noncvine_latent)

#### Non-parametric DTCCKe jitter ####
...
stopCluster(cl) #Stopping cluster
print("Time for running the non-DLCCKe procedure for the simulations:")
noncvine_time_jitter <- end_time-start_time
print(noncvine_time_jitter)

#Exporting the results to csv file
write.csv(df_noncvine_results, file = "df_noncvine_results_Gaussian_X_jitter.csv", row.names = FALSE)

results_noncvine_jitter <- df_noncvine_results%>% group_by(Ordinals_Proportion) %>%
  summarise(meanPower = mean(Cvine_power),meanFDP = mean(Cvine_FDP) )
View(results_noncvine_jitter)
...
for (idx in 1:n_simulations) {
  ...
  print("Time for running the C-vine e-kn procedure:")
  print(time_ekn_cvines)
...
stopCluster(cl) #Stopping cluster
print("Time for running the parametric-DLCCKe procedure for the simulations:")
cvine_time_latent <- end_time-start_time
print(cvine_time_latent)

#Exporting the results to csv file
write.csv(df_cvine_results, file = "df_cvine_results_Gaussian_X_latent.csv", row.names = FALSE)

results_cvine_latent <- df_cvine_results%>% group_by(Ordinals_Proportion) %>%
  summarise(meanPower = mean(Cvine_power),meanFDP = mean(Cvine_FDP) )
View(results_cvine_latent)




#### Parametric DTCCKe Jitter ####
...
stopCluster(cl) #Stopping cluster
print("Time for running the parametric-DLCCKe procedure for the simulations:")
cvine_time_jitter <- end_time-start_time
print(cvine_time_jitter)

#Exporting the results to csv file
write.csv(df_cvine_results, file = "df_cvine_results_Gaussian_X_jitter.csv", row.names = FALSE)

results_cvine_jitter <- df_cvine_results%>% group_by(Ordinals_Proportion) %>%
  summarise(meanPower = mean(Cvine_power),meanFDP = mean(Cvine_FDP) )
View(results_cvine_jitter)
