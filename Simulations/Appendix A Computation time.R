...
ekn_dvines <- function(X, X_cont, column_type, y, dvine_distributions, M=50, M_lasso=10, alpha=0.2, 
  ...
  for(m in 1:M){
    ...
    if(random_evalues==TRUE){
      ...
      #Printing create_dvine_time
      print("create_dvine_time in seconds:")
      print(create_dvine_time)
...
for (l in 1:l_nsim ){
  ...
  for (idx in 1:n_simulations) {
    ...
    cat(sprintf("The knockoff DTLDCKe filter POWER %.2f%% with an FDP of %.2f%%\n", dvine_Power, dvine_FDP))
  ...
  print("Time for running the non-DTLDCKe PROCEDURE (MIN):")
  print(nondvine_time)
  ...
  print("Time for running the PAR-DTLDCKe PROCEDURE (MIN):")
  print(dvine_time)
...
##### Simulations changing p for DLCCKe #####

l_nsim <- 5

vec_p_nonDLCCKe_time <- rep(0,l_nsim )
vec_p_parDLCCKe_time <- rep(0,l_nsim )
...
for (l in 1:l_nsim ){
  ...
  df_column_names_cvine_results <- c("Ordinals_Proportion", "Iteration", "Cvine_power", "Cvine_FDP")
  ...
  for (idx in 1:n_simulations) {
    ...
    # C-vine fitting
    rvine_distributions <- X_Xk_rvine_distributions(X_cont = X_cont_cvine_order, 
                                                    vinecop_family=vinecop_family, 
                                                    n_cores=n_cores)
    ...
    # Adjusting the order of the beta vector to match the C-vine order
    ...
    cat(sprintf("The knockoff Non-DLCCKe filter POWER %.2f%% with an FDP of %.2f%%\n", cvine_Power, cvine_FDP))
    
    # Data frame with the information of this iteration
    data <- data.frame(Ordinals_Proportion = value, Iteration = iteration, Cvine_power = cvine_Power, Cvine_FDP = cvine_FDP)
  ...
  end_time_non <- Sys.time()
  noncvine_time <- as.numeric(difftime(end_time_non, start_time_non, 
                                       units = "mins"))
  
  stopCluster(cl_non) #Stopping cluster
  print("Time for running the NONPARAMETRIC-DLCCKe PROCEDURE (MIN):")
  print(noncvine_time)
  
  vec_p_nonDLCCKe_time[l]<- noncvine_time
  ...
  end_time_par <- Sys.time()
  cvine_time <- as.numeric(difftime(end_time_par, start_time_par, units = "mins"))
  
  stopCluster(cl_par) #Stopping cluster
  print("Time for running the PAR-DLCCKe PROCEDURE (MIN):")
  print(cvine_time)
  vec_p_parDLCCKe_time[l] <- cvine_time
...
#Dataframe with results
df_results_p_varies_DLCCKe <- data.frame(Features=rep(vec_p,2), 
                                  Method=c(rep("NonDLCCKe",l_nsim),rep("DLCCKe",l_nsim)),
                                  Time= c(vec_p_nonDLCCKe_time,vec_p_parDLCCKe_time))

#Exporting the results to csv file
#write.csv(df_results_p_varies_DLCCKe, file = "df_results_p_varies_DLCCKe_1core_8feb26.csv", row.names = FALSE)
write.csv(df_results_p_varies_DLCCKe, file = "df_results_p_varies_DLCCKe_20cores_8feb26.csv", row.names = FALSE)



##### Simulations changing n for DLCCKe #####

l_nsim <- 5

vec_n_nonDLCCKe_time <- rep(0,l_nsim )
vec_n_parDLCCKe_time <- rep(0,l_nsim )
...
for (l in 1:l_nsim ){
  ...
  stopCluster(cl_non) #Stopping cluster
  print("Time for running the non-DLCCKe PROCEDURE (MIN):")
  print(noncvine_time)
  
  vec_n_nonDLCCKe_time[l]<-noncvine_time
  ...
  vec_n_parDLCCKe_time[l] <- cvine_time
  

}

#Dataframe with results
df_results_n_varies_DLCCKe <- data.frame(Features=rep(vec_n,2), 
                                  Method=c(rep("NonDLCCKe",l_nsim),rep("DLCCKe",l_nsim)),
                                  Time= c(vec_n_nonDLCCKe_time,vec_n_parDLCCKe_time))


#Exporting the results to csv file
#write.csv(df_results_n_varies_DLCCKe, file = "df_results_n_varies_DLCCKe_1core_8feb26.csv", row.names = FALSE)
write.csv(df_results_n_varies_DLCCKe, file = "df_results_n_varies_DLCCKe_20cores_8feb26.csv", row.names = FALSE)




##### Simulations changing p, M=1, time for vine fitting and knockoff sampling #####

l_nsim <- 5

vec_p_nonDTLDCKe_time <- rep(0,l_nsim )
vec_p_parDTLDCKe_time <- rep(0,l_nsim )
vec_p_nonDLCCKe_time <- rep(0,l_nsim )
vec_p_parDLCCKe_time <- rep(0,l_nsim )
...
for (l in 1:l_nsim ){
  ...
  for (idx in 1:n_simulations) {
    ...
    # D-vine fitting
    dvine_distributions <- X_Xk_dvine_distributions(X_cont = X_cont_dvine_order, 
                                                    vinecop_family, n_cores)
    ...
    cat(sprintf("The knockoff NON-DTLDCke filter POWER %.2f%% with an FDP of %.2f%%\n", dvine_Power, dvine_FDP))
  ...
  stopCluster(cl_non) #Stopping cluster
  print("Time for running the NONPARAMETRIC-DTLDCKe PROCEDURE (MIN):")
  print(nondvine_time)
  
  vec_p_nonDTLDCKe_time[l]<-nondvine_time
  ...
  print("Time for running the PAR-DTLDCKe PROCEDURE (MIN):")
  print(dvine_time)
  vec_p_parDTLDCKe_time[l] <- dvine_time
...
#Exporting the results to csv file
#write.csv(df_results_p_varies_DLCCKe, file = "df_results_p_varies_DLCCKe_20cores_7feb26.csv", row.names = FALSE)
#write.csv(df_results_p_varies_DLCCKe, file = "df_results_p_varies_DLCCKe_1core_7feb26.csv", row.names = FALSE)



##### Simulations changing n, M=1, time for vine fitting and knockoff sampling #####

l_nsim <- 5

vec_n_nonDTLDCKe_time <- rep(0,l_nsim )
vec_n_parDTLDCKe_time <- rep(0,l_nsim )
vec_n_nonDLCCKe_time <- rep(0,l_nsim )
vec_n_parDLCCKe_time <- rep(0,l_nsim )
...
for (l in 1:l_nsim ){
  ...
  for (idx in 1:n_simulations) {
    ...
    # D-vine fitting
    dvine_distributions <- X_Xk_dvine_distributions(X_cont = X_cont_dvine_order, 
                                                    threshold=0.2, vinecop_family, n_cores)
  ...
  vec_n_nonDTLDCKe_time[l]<-nondvine_time
  ...
  print("Time for running the PAR-DTLDCKe PROCEDURE (MIN):")
  print(dvine_time)
  vec_n_parDTLDCKe_time[l] <- dvine_time
...
#Dataframe with results
#df_results_n_varies_DLCCKe <- data.frame(Features=rep(vec_n,4), 
#                                          Method=c(rep("NonDLCCKe",l_nsim),rep("DLCCKe",l_nsim)),
#                                          Time= c(vec_n_nonDLCCKe_time,vec_n_parDLCCKe_time,))


#Exporting the results to csv file
#write.csv(df_results_n_varies_DLCCKe, file = "df_results_n_varies_DLCCKe_20cores_7feb26.csv", row.names = FALSE)
#write.csv(df_results_n_varies_DLCCKe, file = "df_results_n_varies_DLCCKe_1core_7feb26.csv", row.names = FALSE)
